@ Разработать подпрограмму, преобразующую знаковое целое 32-разрядное число,
@ хранящееся в регистре R0 в ASCII-строку и записать её в память начиная с
@ адреса, хранящегося в регистре R1.
...
convert:
	...
	cmp 	r0, #0						@ проверяем знак
	itt		lt							@ если <0
	movlt 	r4, #1						@ устанавливаем флаг отрицательного числа 1
	andlt 	r0, r3						@ сделать r0 положительным числом маской
convert_get_digits:						@ эта функция должна записать все цифры в стек, начиная с последней
    mov 	r3, #0
    push 	{r3}						@ в конце строки нужен будет '\0'
convert_get_digits_while:
	mov 	r3, #10
	udiv 	r2, r0, r3					@ r2 = r0 / 10
	mls 	r3, r3, r2, r0				@ r3 = r0 - (r2 * 10) - остаток от деления r0 / 10
	add 	r3, '0'						@ прибавить код символа '0'
	mov 	r0, r2						@ r0 = r2 = r0/10
	push 	{r3}						@ отправляем остаток от деления в стек
    teq 	r0, #0						@ если r0=0, то больше цифр нет
    bne		convert_get_digits_while	@ иначе возвращаемся в цикл
convert_print_digits:					@ эта функция считывает стек и пишет цифры по адрессу
	mov 	r3, '-'
	cmp 	r4, #0						@ перед циклом записываем знак, если он есть
	itt 	hi							@ если флаг знака числа больше 0, то число отрицательное
	strhi	r3, [r1]					@ записываем знак
	addhi 	r1, #1						@ увеличиваем следующий адрес на 1
convert_print_digits_while:
	pop		{r3}						@ берём символ из стека
	strb	r3, [r1]					@ записываем 1 байт по адресу
	add 	r1, #1						@ увеличиваем следующий адрес на 1
	teq		r3, #0						@ если записали '\0', то заканчиваем запись
	bne 	convert_print_digits_while	@ иначе возвращаемся в цикл
convert_end:
	pop		{r4}						@ возвращаем все регистры
	pop		{r3}
	pop		{r2}
	pop		{r1}
	pop		{r0}
